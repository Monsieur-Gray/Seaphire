...
fn damedame(
    ...
    if condition.len() == 1 && fetch_bool(condition.get(0).unwrap()).unwrap() {
        ...
        let eval_expr = match (oper, ans) {
            ...
            (Builtins::CMP(CompOp::UNEQUAL), Some(Ordering::Less) | Some(Ordering::Greater)) => {
                true
            }
            (
                Builtins::CMP(CompOp::GREATER_EQ),
                Some(Ordering::Greater) | Some(Ordering::Equal),
            ) => true,
            (Builtins::CMP(CompOp::LESS_EQ), Some(Ordering::Less) | Some(Ordering::Equal)) => true,
